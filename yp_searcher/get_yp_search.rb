...
query = "/#{CGI.escape location}search?q=#{CGI.escape query}"
...
(1..(options[:pages] || 1)).each do |page| 
  ...
  curl = "curl -s -A Mozilla 'http://www.yellowpages.com/washington-dc/beer-stuff?g=Washington%2C+DC&q=beer+stuff#{query}'"
  resp = %x{#{curl}}

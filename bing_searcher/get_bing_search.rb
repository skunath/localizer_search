...
END
  ...
  opts.on("-l", '--location [DATE RANGE]', 'Show results for date range. See below for options.') {|dr| options[:location] = dr }
...
query = "/local/default.aspx?what=#{CGI.escape query}&where=Baltimore%2c+Maryland"
...
(1..(options[:pages] || 1)).each do |page| 
  ...
  curl = "curl -s -A Mozilla 'http://www.bing.com/#{query}'"
  resp = %x{#{curl}}
  ...
  doc.search('ol li.llsResultItem').each_with_index {|li, index|
    puts li.at("div.ecHeader").content
    puts li.at("div.ecPaddingBottom").content
